...
import type { Product, QuantityMap, QuoteFormData, Preset } from '../types'
        ...
        <div className="modal-overlay" onClick={() => setPreviewPreset(null)}>
          <div className="modal-box" onClick={(e) => e.stopPropagation()} style={{ maxWidth: 480 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, marginBottom: 16 }}>
              <div>
                ...
                {previewPreset.description && (
                  <p style={{ fontSize: 13, color: 'var(--text-2)', marginTop: 3 }}>{previewPreset.description}</p>
                )}
              </div>
                ...
                return (
                  <div key={item.id} className="preset-preview-item">
                    <div style={{ flex: 1 }}>
                      ...
                      {product?.part_number && (
                        <span className="part-number" style={{ marginTop: 3, display: 'inline-block' }}>{product.part_number}</span>
                      )}
                    </div>
            ...
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
              ...
              <button className="btn btn-secondary" onClick={() => { onApply(previewPreset); setPreviewPreset(null) }}>
                Apply preset
              </button>
            </div>
          </div>
...
export default function QuoteBuilder({
  ...
  quoteForm, setQuoteForm, onSave, showToast, presets, onApplyPreset,
}: Props) {
                  ...
                  <table className="product-table">
                    ...
                    <tbody>
                      {items.map((p) => {
                        ...
                        return (
                          <tr key={p.id}>
                            <td><div style={{ fontWeight: qty > 0 ? 500 : 400 }}>{p.description}</div>
